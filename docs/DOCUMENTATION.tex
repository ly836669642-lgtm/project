...
locates the affected stretch directly by searching the path's arc-length
ordering, which keeps replanning cheap and responsive even as the route
grows. The detoured section is re-run through the same curvature- and
acceleration-limited speed profile as the base path, so it stays just as
driveable. Once the obstacle is reported clear, the planner simply
reverts to the unmodified base path.
